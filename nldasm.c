...
ST_FUNC int asm_get_local_label_name(NILDOState *s1, unsigned int n)
{
    char buf[64];
    TokenSym *ts;
...
static int nld_assemble_internal(NILDOState *s1, int do_preprocess, int global);
static Sym* asm_new_label(NILDOState *s1, int label, int is_local);
static Sym* asm_new_label1(NILDOState *s1, int label, int is_local, int sh_num, int value);
...
static Sym* asm_section_sym(NILDOState *s1, Section *sec)
{
    char buf[100];
...
static void asm_expr_unary(NILDOState *s1, ExprValue *pe)
{
    Sym *sym;
    int op, label;
    uint64_t n;
    ...
    case TOK_PPNUM:
        ...
        if (*p == 'b' || *p == 'f') {
            ...
            if (*p == 'b') {
                ...
                if (sym && (!sym->c || elfsym(sym)->st_shndx == SHN_UNDEF))
                    sym = sym->prev_tok;
                if (!sym)
                    nld_error("local label '%d' not found backward", n);
            } else {
                /* forward */
                if (!sym || (sym->c && elfsym(sym)->st_shndx != SHN_UNDEF)) {
	    ...
	    pe->pcrel = 0;
            ...
            nld_error("invalid number syntax");
        }
        next();
        break;
    case '+':
    ...
    case '~':
        ...
        asm_expr_unary(s1, pe);
        if (pe->sym)
            nld_error("invalid operation with label");
        if (op == '-')
            pe->v = -pe->v;
        else
            pe->v = ~pe->v;
    ...
    default:
		...
		pe->pcrel = 0;
            ...
            nld_error("bad expression syntax [%s]", get_tok_str(tok, &tokc));
        }
        break;
    }
}
    
static void asm_expr_prod(NILDOState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static void asm_expr_logic(NILDOState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static inline void asm_expr_sum(NILDOState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static inline void asm_expr_cmp(NILDOState *s1, ExprValue *pe)
{
    int op;
    ExprValue e2;
...
static Sym* asm_new_label1(NILDOState *s1, int label, int is_local,
	   ...
	   definitions won't have VT_EXTERN set.  */
        if (esym && esym->st_shndx != SHN_UNDEF) {
            ...
            if (!(sym->type.t & VT_EXTERN))
                nld_error("assembler label '%s' already defined",
...
static Sym* set_symbol(NILDOState *s1, int label)
{
    long n;
    ExprValue e;
    Sym *sym;
...
static void use_section1(NILDOState *s1, Section *sec)
{
...
static void use_section(NILDOState *s1, const char *name)
{
    Section *sec;
...
static void push_section(NILDOState *s1, const char *name)
{
...
static void pop_section(NILDOState *s1)
{
    Section *prev = cur_text_section->prev;
    if (!prev)
        nld_error(".popsection without .pushsection");
    cur_text_section->prev = NULL;
    use_section1(s1, prev);
}

static void asm_parse_directive(NILDOState *s1, int global)
{
    ...
    case TOK_ASMDIR_space:
        ...
        {
            if (n < 0 || n > 30)
                nld_error("invalid p2align, must be between 0 and 30");
            n = 1 << n;
    ...
    case TOK_ASMDIR_fill:
        {
            ...
            if (repeat < 0) {
                nld_error("repeat < 0; .fill ignored");
                break;
            }
            size = 1;
            val = 0;
    ...
    case TOK_ASMDIR_rept:
        {
            ...
            while (next(), tok != TOK_ASMDIR_endr) {
                if (tok == CH_EOF)
                    nld_error("we at end of file, .endr not found");
            ...
            while (repeat-- > 0) {
                nld_assemble_internal(s1, (parse_flags & PARSE_FLAG_PREPROCESS),
				      global);
    ...
    case TOK_ASMDIR_org:
		...
		n += esym->st_value;
            ...
            if (n < ind)
                nld_error("attempt to .org backwards");
            v = 0;
            size = n - ind;
            goto zero_pad;
        }
    ...
    case TOK_ASMDIR_file:
        {
            ...
            if (s1->warn_unsupported)
                nld_warning("ignoring .file %s", filename);
    ...
    case TOK_ASMDIR_ident:
        {
            ...
            if (s1->warn_unsupported)
                nld_warning("ignoring .ident %s", ident);
    ...
    case TOK_ASMDIR_size:
        { 
            ...
            if (!sym) {
                nld_error("label not found: %s", get_tok_str(tok, NULL));
            }

            /* XXX .size name,label2-label1 */
            if (s1->warn_unsupported)
                nld_warning("ignoring .size %s,*", get_tok_str(tok, NULL));
    ...
    case TOK_ASMDIR_type:
        { 
            ...
            if (!strcmp(newtype, "function") || !strcmp(newtype, "STT_FUNC")) {
                ...
                nld_warning("change type of '%s' from 0x%x to '%s' ignored", 
    ...
    case TOK_ASMDIR_previous:
        { 
            ...
            if (!last_text_section)
                nld_error("no previous section referenced");
            sec = cur_text_section;
...
#endif
    default:
        nld_error("unknown assembler directive '.%s'", get_tok_str(tok, NULL));
        break;
    }
}
...
ST_FUNC int nld_assemble(NILDOState *s1, int do_preprocess)
{
    ...
    ind = cur_text_section->data_offset;
    nocode_wanted = 0;
    ret = nld_assemble_internal(s1, do_preprocess, 1);
    cur_text_section->data_offset = ind;
    nld_debug_end(s1);
    return ret;
}
...
static void nld_assemble_inline(NILDOState *s1, char *str, int len, int global)
{
    const int *saved_macro_ptr = macro_ptr;
    int dotid = set_idnum('.', IS_ID);

    nld_open_bf(s1, ":asm:", len);
    memcpy(file->buffer, str, len);
    macro_ptr = NULL;
    nld_assemble_internal(s1, 0, global);
    nld_close();
                               ...
                               CString *out_str, CString *in_str)
{
    ...
    for(;;) {
		...
		*str == 'P')
            ...
            if (index < 0)
                nld_error("invalid operand reference after %%");
            op = &operands[index];
            sv = *op->vt;
            if (op->reg >= 0) {
                sv.r = op->reg;
    ...
    if (tok != ':') {
	    ...
	    CString astr;
            if (nb_operands >= MAX_ASM_OPERANDS)
                nld_error("too many asm operands");
	    ...
	    parse_mult_str(&astr, "string constant");
            op->constraint = nld_malloc(astr.size);
...
ST_FUNC void asm_instr(void)
{
    ...
    /* assemble the string with nld internal assembler */
    nld_assemble_inline(nld_state, astr1.data, astr1.size - 1, 0);
...
ST_FUNC void asm_global_instr(void)
{
    ...
    /* assemble the string with nld internal assembler */
    nld_assemble_inline(nld_state, astr.data, astr.size - 1, 1);

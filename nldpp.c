...
static const char nld_keywords[] = 
...
{
    if (tok != c)
        nld_error("'%c' expected (got \"%s\")", c, get_tok_str(tok, &tokc));
    next();
}

ST_FUNC void expect(const char *msg)
{
    nld_error("%s expected", msg);
}
...
#ifndef USE_TAL
#define tal_free(al, p) nld_free(p)
#define tal_realloc(al, p, size) nld_realloc(p, size)
...
{
    TinyAlloc *al = nld_mallocz(sizeof(TinyAlloc));
    al->p = al->buffer = nld_malloc(size);
    al->limit = limit;
    al->size = size;
    if (pal) *pal = al;
    return al;
...
static TokenSym *tok_alloc_new(TokenSym **pts, const char *str, int len)
{
    ...
    if (tok_ident >= SYM_FIRST_ANOM) 
        nld_error("memory full (symbols)");

    /* expand token table if needed */
    i = tok_ident - TOK_IDENT;
    if ((i % TOK_ALLOC_INCR) == 0) {
        ptable = nld_realloc(table_ident, (i + TOK_ALLOC_INCR) * sizeof(TokenSym *));
        table_ident = ptable;
    }
        ...
        if (c == '\n') {
            ...
            for(;;) {
                ...
                if (c == '*') {
                    ...
                    p = file->buf_ptr;
                    if (c == CH_EOF)
                        nld_error("unexpected end of file in comment");
                    if (c == '\\') {
    ...
    for(;;) {
        ...
        if (c == sep) {
            ...
            unterminated_string:
                ...
                nld_error("missing terminating %c character", sep);
            } else if (c == '\\') {
                /* escape : just skip \[\r]\n */
                PEEKC_EOB(c, p);
                if (c == '\n') {
...
ST_INLN void define_push(int v, int macro_type, int *str, Sym *first_arg)
{
	...
	nld_warning("%s redefined", get_tok_str(v, NULL));
}
...
ST_FUNC void label_pop(Sym **ptop, Sym *slast, int keep)
{
    ...
    for(s = *ptop; s != slast; s = s1) {
        ...
        if (s->r == LABEL_DECLARED) {
            nld_warning("label '%s' declared but not used", get_tok_str(s->v, NULL));
        } else if (s->r == LABEL_FORWARD) {
                nld_error("label '%s' used but not defined",
...
/* fake the nth "#if defined test_..." for nld -dt -run */
static void maybe_run_test(NILDOState *s)
{
    const char *p;
...
static int expr_preprocess(void)
{
    ...
    while (tok != TOK_LINEFEED && tok != TOK_EOF) {
        ...
        if (tok == TOK_DEFINED) {
            ...
            if (tok < TOK_IDENT)
                expect("identifier");
            if (nld_state->run_test)
                maybe_run_test(nld_state);
            c = define_find(tok) != 0;
            if (t == '(') {
                next_nomacro();
                if (tok != ')')
...
ST_FUNC void parse_define(void)
{
    ...
    if (v < TOK_IDENT || v == TOK_DEFINED)
        nld_error("invalid macro name '%s'", get_tok_str(tok, &tokc));
    ...
    if (tok == '(') {
        ...
        if (tok != ')') for (;;) {
            ...
            if (varg < TOK_IDENT)
        bad_list:
                nld_error("bad macro parameter list");
    ...
    if (3 == spc)
bad_twosharp:
        nld_error("'##' cannot appear at either end of macro");
    define_push(v, t, tok_str_dup(&tokstr_buf), first);
}

static CachedInclude *search_cached_include(NILDOState *s1, const char *filename, int add)
{
    ...
    e = nld_malloc(sizeof(CachedInclude) + strlen(filename));
...
static void pragma_parse(NILDOState *s1)
{
    next_nomacro();
    if (tok == TOK_push_macro || tok == TOK_pop_macro) {
        ...
        if (s)
            ...
            nld_warning("unbalanced #pragma pop_macro");
        pp_debug_tok = t, pp_debug_symv = v;

    } else if (tok == TOK_once) {
        search_cached_include(s1, file->filename, 1)->once = pp_once;

    } else if (s1->output_type == NILDO_OUTPUT_PREPROCESS) {
        /* nld -E: keep pragmas below unchanged */
        unget_tok(' ');
        ...
        if (tok != TOK_STR)
            goto pragma_err;
        p = nld_strdup((char *)tokc.str.data);
        next();
        if (tok != ')')
            goto pragma_err;
        if (t == TOK_lib) {
            ...
            if (t == TOK_option)
                nld_set_options(s1, p);
            nld_free(p);
        }

    } else if (s1->warn_unsupported) {
        nld_warning("#pragma %s is ignored", get_tok_str(tok, &tokc));
    }
    return;

pragma_err:
    nld_error("malformed #pragma directive");
    return;
}
 ...
 redo:
    ...
    case TOK_INCLUDE_NEXT:
	    ...
	    if ((len < 2 || ((buf[0] != '"' || buf[len-1] != '"') &&
			     (buf[0] != '<' || buf[len-1] != '>'))))
	        nld_error("'#include' expects \"FILENAME\" or <FILENAME>");
	    c = buf[len-1];
        ...
        if (s1->include_stack_ptr >= s1->include_stack + INCLUDE_STACK_SIZE)
            nld_error("#include recursion too deep");
        ...
        *s1->include_stack_ptr = file;
        ...
        for (; i < n; ++i) {
            ...
            if (i == 0) {
                ...
                pstrncpy(buf1, path, nld_basename(path) - path);
        ...
        nld_error("include file '%s' not found", buf);
include_done:
        break;
    case TOK_IFNDEF:
        c = 1;
    ...
    do_ifdef:
        next_nomacro();
        if (tok < TOK_IDENT)
            nld_error("invalid argument for '#if%sdef'", c ? "n" : "");
        if (is_bof) {
            if (c) {
#ifdef INC_DEBUG
    ...
    do_if:
        if (s1->ifdef_stack_ptr >= s1->ifdef_stack + IFDEF_STACK_SIZE)
            nld_error("memory full (ifdef)");
        *s1->ifdef_stack_ptr++ = c;
        goto test_skip;
    case TOK_ELSE:
        if (s1->ifdef_stack_ptr == s1->ifdef_stack)
            nld_error("#else without matching #if");
        if (s1->ifdef_stack_ptr[-1] & 2)
            nld_error("#else after #else");
        c = (s1->ifdef_stack_ptr[-1] ^= 3);
        goto test_else;
    case TOK_ELIF:
        if (s1->ifdef_stack_ptr == s1->ifdef_stack)
            nld_error("#elif without matching #if");
        c = s1->ifdef_stack_ptr[-1];
        if (c > 1)
            nld_error("#elif after #else");
    ...
    case TOK_ENDIF:
        if (s1->ifdef_stack_ptr <= file->ifdef_stack_ptr)
            nld_error("#endif without matching #if");
        s1->ifdef_stack_ptr--;
        ...
        if (tok != TOK_LINEFEED) {
            if (tok == TOK_STR) {
                if (file->true_filename == file->filename)
                    file->true_filename = nld_strdup(file->filename);
    ...
    case TOK_WARNING:
        ...
        *q = '\0';
        if (c == TOK_ERROR)
            nld_error("#error %s", buf);
        else
            nld_warning("#warning %s", buf);
        break;
    case TOK_PRAGMA:
        pragma_parse(s1);
        break;
    ...
    default:
        ...
        if (tok == '!' && is_bof)
            ...
            goto ignore;
        nld_warning("Ignoring unknown preprocessing directive #%s", get_tok_str(tok, &tokc));
    ignore:
...
static void parse_escape_string(CString *outstr, const uint8_t *buf, int is_long)
{
    ...
    for(;;) {
        ...
        if (c == '\\') {
            ...
            invalid_escape:
                if (c >= '!' && c <= '~')
                    nld_warning("unknown escape sequence: \'\\%c\'", c);
                else
                    nld_warning("unknown escape sequence: \'\\x%x\'", c);
                break;
            }
        ...
        invalid_utf8_sequence:
            nld_warning("ill-formed UTF-8 subsequence starting with: \'\\x%x\'", c);
            c = 0xFFFD;
            p += skip;
            goto add_char_nonext;
...
static void parse_string(const char *s, int len)
{
    ...
    if (sep == '\'') {
        ...
        if (n < 1)
            nld_error("empty character constant");
        if (n > 1)
            nld_warning("multi-character character constant");
        for (c = i = 0; i < n; ++i) {
            if (is_long)
    ...
    if (ch == '.' ||
        ...
        *q = '\0';
        ...
        for(;;) {
            ...
            if (t == 'L') {
                if (lcount >= 2)
                    nld_error("three 'l's in integer constant");
                if (lcount && *(p - 1) != ch)
                    nld_error("incorrect integer suffix: %s", p1);
                lcount++;
                ch = *p++;
            } else if (t == 'U') {
                if (ucount >= 1)
                    nld_error("two 'u's in integer constant");
                ucount++;
                ch = *p++;
            } else {
                break;
        ...
        if (ov)
            nld_warning("integer constant overflow");
...
maybe_newline:
    ...
    default:
        ...
        if (parse_flags & PARSE_FLAG_ASM_FILE)
            goto parse_simple;
        nld_error("unrecognized character \\x%02x", c);
        break;
    }
    tok_flags = 0;
keep_tok_flags:
...
static int paste_tokens(int t1, CValue *v1, int t2, CValue *v2)
{
    ...
    nld_open_bf(nld_state, ":paste:", cstr.size);
    ...
    for (;;) {
        ...
        if (is_space(tok))
            continue;
        nld_warning("pasting \"%.*s\" and \"%s\" does not give a valid"
    ...
    Sym *s)
{
    ...
    empty_arg:
                ...
                if (!args && !sa && tok == ')')
                    break;
                if (!sa)
                    nld_error("macro '%s' used with too many args",
                          get_tok_str(s->v, 0));
                tok_str_new(&str);
                parlevel = spc = 0;
            ...
            if (sa) {
                nld_error("macro '%s' used with too few args",
                      get_tok_str(s->v, 0));
            }
...
ST_FUNC void preprocess_start(NILDOState *s1, int is_asm)
{
    ...
    cstr_cat(&cstr, "\"", 0);
    nld_define_symbol(s1, "__BASE_FILE__", cstr.data);
    ...
    if (cstr.size) {
        *s1->include_stack_ptr++ = file;
	nld_open_bf(s1, "<command line>", cstr.size);
	memcpy(file->buffer, cstr.data, cstr.size);
    }
    cstr_free(&cstr);

    if (is_asm)
        nld_define_symbol(s1, "__ASSEMBLER__", NULL);
...
static void pp_line(NILDOState *s1, BufferedFile *f, int level)
{
...
static void define_print(NILDOState *s1, int v)
{
    FILE *fp;
    Sym *s;
...
static void pp_debug_defines(NILDOState *s1)
{
    int v, t;
    const char *vs;
    FILE *fp;
...
static void pp_debug_builtins(NILDOState *s1)
{
    int v;
...
ST_FUNC int nld_preprocess(NILDOState *s1)
{
    ...
    /* Credits to Fabrice Bellard's initial revision to demonstrate its
       ...
       given as decimals. nld -E -P10 will do. */

...
usage: ctec [options] [I<infile1> I<infile2>...] [B<-run> I<infile> I<args>...]
...
=item C\<B<ctec -run a.c>\>

Compile F<a.c> and execute it directly


=item C\<B<ctec -run a.c arg1>\>

Compile a.c and execute it directly. arg1 is given as first argument to
the C<main()> of a.c.


=item C\<B<ctec a.c -run b.c arg1>\>
...
=item C\<B<ctec -r -o ab.o a.c b.c>\>
...
CTEC can be invoked from I<scripts>, just as shell scripts. You just
need to add C<#!/usr/local/bin/ctec -run> at the start of your C source:
...
CTEC can read C source code from I<standard input> when B<-> is used in 
...
script, several CTEC options can be given I<after> the
...
and F<PREFIX/lib/ctec/include>. (F<PREFIX> is usually
...
found (default is F<PREFIX/lib/ctec>).
...
Note: B<-b> is only available on i386 when using libctec for the moment.

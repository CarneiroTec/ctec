...
/* NILDO module system - PSR-4 style namespacing */
static char current_module_name[256];  /* current module name (empty if global) */
static int in_module_block;             /* 1 if inside Módulo { } block */
...
static void gen_inline_functions(NILDOState *s);
...
/* compiling intel long double natively */
    ...
    && (defined NILDO_TARGET_I386 || defined NILDO_TARGET_X86_64)
# define NILDO_IS_NATIVE_387
#endif
...
{
    if (pvtop != vtop)
        nld_error("internal compiler error: vstack leak (%d)", vtop - pvtop);
}
                            ...
                            int can_add_underscore)
{
    ...
    if (!sym->c) {
...
#endif
        if (nld_state->leading_underscore && can_add_underscore) {
            buf1[0] = '_';
        ...
        /* NILDO module system: prefix symbol with module name */
        if (current_module_name[0] != '\0' && !(t & VT_PRIVATE)) {
            static char module_prefixed_name[512];
            snprintf(module_prefixed_name, sizeof(module_prefixed_name), 
                     "%s_%s", current_module_name, name);
            name = module_prefixed_name;
        }
...
static Sym *__sym_malloc(void)
{
    ...
    sym_pool = nld_malloc(SYM_POOL_NB * sizeof(Sym));
...
static void patch_type(Sym *sym, CType *type)
{
    if (!(type->t & VT_EXTERN)) {
        if (!(sym->type.t & VT_EXTERN))
            nld_error("redefinition of '%s'", get_tok_str(sym->v, NULL));
    ...
    if (!is_compatible_types(&sym->type, type)) {
        nld_error("incompatible types for redefinition of '%s'",
                  get_tok_str(sym->v, NULL));
        ...
        if ((type->t & VT_STATIC) && !static_proto && !(type->t & VT_INLINE))
            nld_warning("static storage ignored for redefinition of '%s'",
                get_tok_str(sym->v, NULL));
        ...
        if ((sym->type.t & VT_ARRAY) && type->ref->c >= 0) {
            ...
            if (sym->type.ref->c < 0)
                ...
                nld_error("conflicting type for '%s'", get_tok_str(sym->v, NULL));
        }
        if ((type->t ^ sym->type.t) & VT_STATIC)
            nld_warning("storage mismatch for redefinition of '%s'",
                get_tok_str(sym->v, NULL));
    }
}
...
static void patch_storage(Sym *sym, AttributeDef *ad, CType *type)
{
    ...
    if (sym->a.dllimport != ad->a.dllimport)
        nld_error("incompatible dll linkage for redefinition of '%s'",
            get_tok_str(sym->v, NULL));
...
static void gen_opic(int op)
{
    ...
    if (c1 && c2) {
        ...
        case TOK_UMOD:
            ...
            if (l2 == 0) {
                if (const_wanted)
                    nld_error("division by zero in constant");
                goto general_case;
            }
            switch(op) {
...
static void check_comparison_pointer_types(SValue *p1, SValue *p2, int op)
{
    ...
    if ((is_integer_btype(bt1) || is_integer_btype(bt2)) && op != '-') {
        if (op != TOK_LOR && op != TOK_LAND )
            nld_warning("comparison between pointer and integer");
        return;
    }
    ...
    invalid_operands:
        nld_error("invalid operands to binary %s", get_tok_str(op, NULL));
    }
    ...
    if (!is_compatible_types(&tmp_type1, &tmp_type2)) {
        ...
        if (op == '-')
            ...
            nld_warning("comparison of distinct pointer types lacks a cast");
    }
}
...
redo:
    ...
    if (bt1 == VT_STRUCT || bt2 == VT_STRUCT) {
        nld_error("operation on a struct");
	...
	goto redo;
        ...
        if (bt1 == VT_PTR && bt2 == VT_PTR) {
            if (op != '-')
                nld_error("cannot use pointers here");
            check_comparison_pointer_types(vtop - 1, vtop, op);
            ...
            if (vtop[-1].type.t & VT_VLA)
                ...
                if (u < 0)
                    nld_error("unknown array element size");
#if PTR_SIZE == 8
                vpushll(u);
#else
...
/* #ifdef CONFIG_NILDO_BCHECK
    ...
    */
            ...
            if (nld_state->do_bounds_check && !const_wanted) {
        ...
        if (op != '+' && op != '-' && op != '*' && op != '/' &&
            ...
            nld_error("invalid operands for binary operation");
        goto std_op;
...
#if !defined NILDO_IS_NATIVE && !defined NILDO_IS_NATIVE_387
        c &= dbt != VT_LDOUBLE;
#endif
        if (c) {
...
#error
                ...
                if (sbt == VT_PTR) {
                    vtop->type.t = VT_INT;
                    nld_warning("nonportable conversion from pointer to char/short");
                }
...
static void gen_assign_cast(CType *dt)
{
	...
	if (sbt == VT_VOID && dbt == VT_VOID)
		...
		void func2() {
	      ...
	      */
    	    ...
    	    nld_error("cannot cast from/to void");
    }
    if (dt->t & VT_CONSTANT)
        nld_warning("assignment of read-only location");
    switch(dbt) {
    case VT_PTR:
        ...
        if (is_integer_btype(sbt)) {
            nld_warning("assignment makes pointer from integer without a cast");
            goto type_ok;
        }
        ...
        if (sbt == VT_FUNC) {
            if ((type1->t & VT_BTYPE) != VT_VOID &&
                ...
                nld_warning("assignment from incompatible pointer type");
            goto type_ok;
        }
        if (sbt != VT_PTR)
            goto error;
        ...
        if ((!(type1->t & VT_CONSTANT) && (type2->t & VT_CONSTANT)) ||
            ...
            nld_warning("assignment discards qualifiers from pointer target type");
        break;
    case VT_BYTE:
    case VT_SHORT:
    case VT_INT:
    case VT_LLONG:
        if (sbt == VT_PTR || sbt == VT_FUNC) {
            nld_warning("assignment makes integer from pointer without a cast");
    ...
    case_VT_STRUCT:
        ...
        error:
            ...
            nld_error("cannot cast '%s' to '%s'", buf1, buf2);
        }
        break;
    }
 type_ok:
...
redo:
    ...
    while (tok != ')') {
	    ...
	    parse_mult_str(&astr, "section name");
            ad->section = find_section(nld_state, (char *)astr.data);
            skip(')');
	    cstr_free(&astr);
            break;
        case TOK_ALIAS:
	...
	case TOK_VISIBILITY:
        ...
        case TOK_ALIGNED:
            if (tok == '(') {
                ...
                n = expr_const();
                if (n <= 0 || (n & (n - 1)) != 0) 
                    nld_error("alignment must be a positive power of two");
                skip(')');
            } else {
                n = MAX_ALIGN;
            }
	    ...
	    if (n != 1 << (ad->a.aligned - 1))
	      nld_error("alignment of %d is larger than implemented", n);
            break;
        case TOK_PACKED:
            ad->a.packed = 1;
            break;
        case TOK_WEAK:
            ...
            break;
        case TOK_UNUSED:
            /* currently, no need to handle it because nld does not
        ...
        case TOK_MODE:
            ...
            switch(tok) {
                ...
                case TOK_MODE_word:
                    ...
                    break;
                default:
                    nld_warning("__mode__(%s) not supported\n", get_tok_str(tok, NULL));
                    break;
            }
            next();
            skip(')');
        ...
        case TOK_DLLIMPORT:
            ...
            break;
        default:
            if (nld_state->warn_unsupported)
                nld_warning("'%s' attribute ignored", get_tok_str(t, NULL));
            /* skip parameters */
            if (tok == '(') {
                int parenthesis = 0;
                do {
...
static void struct_layout(CType *type, AttributeDef *ad)
{
    ...
    int pcc = !nld_state->ms_bitfields;
    int pragma_pack = *nld_state->pack_stack_ptr;
    Sym *f;
    ...
    if (tok != '{') {
        ...
        if (s && (s->sym_scope == local_scope || tok != '{')) {
            ...
            if (u == VT_ENUM && IS_ENUM(s->type.t))
                goto do_decl;
            nld_error("redefinition of '%s'", get_tok_str(v, NULL));
        }
    } else {
        v = anon_sym++;
    }
...
do_decl:
    ...
    if (tok == '{') {
        next();
        if (s->c != -1)
            nld_error("struct/union/enum already defined");
        /* cannot be empty */
	    ...
	    CType t;
            ...
            for(;;) {
                ...
                ss = sym_find(v);
                if (ss && !local_stack)
                    nld_error("redefinition of enumerator '%s'",
                              get_tok_str(v, NULL));
                next();
                if (tok == '=') {
                    next();
		    ...
		    continue;
                ...
                while (1) {
		    if (flexible)
		        nld_error("flexible array member '%s' not at the end of struct",
                              get_tok_str(v, NULL));
                    bit_size = -1;
                    v = 0;
                    type1 = btype;
			...
			if (tok != ';')
			    ...
			    if ((u == VT_STRUCT) && (type1.t & VT_ARRAY) && c)
			        ...
			        nld_error("field '%s' has incomplete type",
                                      get_tok_str(v, NULL));
                        }
                        if ((type1.t & VT_BTYPE) == VT_FUNC ||
                            (type1.t & VT_STORAGE))
                            nld_error("invalid type for '%s'", 
                                  get_tok_str(v, NULL));
                    }
                    if (tok == ':') {
                        next();
                        ...
                        if (bit_size < 0)
                            nld_error("negative width in bit-field '%s'", 
                                  get_tok_str(v, NULL));
                        if (v && bit_size == 0)
                            nld_error("zero width for bit-field '%s'", 
                                  get_tok_str(v, NULL));
			parse_attribute(&ad1);
                    }
                    ...
                    if (bit_size >= 0) {
                        ...
                        if (bt != VT_INT && 
                            ...
                            bt != VT_BOOL &&
                            bt != VT_LLONG)
                            nld_error("bitfields must have scalar type");
                        bsize = size * 8;
                        if (bit_size > bsize) {
                            nld_error("width of '%s' exceeds its type",
                                  get_tok_str(v, NULL));
                            ...
                            nld_error("field width 64 not implemented");
                        } else {
    ...
    while(1) {
        ...
        basic_type1:
            if (u == VT_SHORT || u == VT_LONG) {
                if (st != -1 || (bt != -1 && bt != VT_INT))
                    tmbt: nld_error("too many basic types");
                st = u;
            } else {
        ...
        case TOK_SIGNED:
            if ((t & (VT_DEFSIGN|VT_UNSIGNED)) == (VT_DEFSIGN|VT_UNSIGNED))
                nld_error("signed and unsigned modifier");
            t |= VT_DEFSIGN;
            next();
            typespec_found = 1;
            break;
       ...
       storage:
            if (t & (VT_EXTERN|VT_STATIC|VT_TYPEDEF) & ~g)
                nld_error("multiple storage classes");
            t |= g;
            next();
            break;
        case TOK_INLINE:
            ...
            for(;;) {
                ...
                if (l != FUNC_OLD) {
                    ...
                    if ((pt.t & VT_BTYPE) == VT_VOID)
                        nld_error("parameter declared as void");
	...
	int saved_nocode_wanted = nocode_wanted;
		...
		gexpr();
            ...
            if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
                n = vtop->c.i;
                if (n < 0)
                    nld_error("invalid array size");
            } else {
                if (!is_integer_btype(vtop->type.t & VT_BTYPE))
                    nld_error("size of variable length array should be an integer");
                t1 = VT_VLA;
            }
        }
        skip(']');
        ...
        if (type->t == VT_FUNC)
            nld_error("declaration of an array of functions");
        t1 |= type->t & VT_VLA;
...
static void gfunc_param_typed(Sym *func, Sym *arg)
{
    ...
    if (func_type == FUNC_OLD ||
        ...
        nld_error("too many arguments to function");
    } else {
        type = arg->type;
...
static void parse_builtin_params(int nc, const char *args)
{
	...
	switch (c) {
	    ...
	    default: nld_error("internal error"); break;
	}
    }
    skip(')');
    if (nc)
...
ST_FUNC void unary(void)
{
 ...
 push_tokc:
    ...
    str_init:
        if (nld_state->warn_write_strings)
            t |= VT_CONSTANT;
        type.t = t;
        mk_pointer(&type);
        type.t |= VT_ARRAY;
    ...
    case '+':
        ...
        unary();
        if ((vtop->type.t & VT_BTYPE) == VT_PTR)
            nld_error("pointer not accepted for unary plus");
    ...
    case TOK_ALIGNOF:
        ...
        if (t == TOK_SIZEOF) {
            if (!(type.t & VT_VLA)) {
                if (size < 0)
                    nld_error("sizeof applied to an incomplete type");
                vpushs(size);
            } else {
                vla_runtime_type_size(&type, &align);
            }
    ...
    case TOK_builtin_return_address:
        {
            ...
            skip('(');
            if (tok != TOK_CINT) {
                nld_error("%s only takes positive integers",
	...
	parse_builtin_params(0, "ee");
        ...
        if (r == VT_LLOCAL)
            r = VT_LOCAL;
        if (r != VT_LOCAL)
            nld_error("__builtin_va_start expects a local variable");
        vtop->r = r;
	vtop->type = char_pointer_type;
	vtop->c.i += 8;
	vstore();
	...
	if (!str) {
	    ...
	    nld_error("type '%s' does not match any association", buf);
	}
	begin_macro(str, 1);
	next();
	expr_eq();
    ...
    tok_identifier:
        ...
        if (t < TOK_UIDENT)
            expect("identifier");
        s = sym_find(t);
        /* NILDO module system: try module.symbol resolution */
        if (!s && tok == '.') {
            const char *module_name = get_tok_str(t, NULL);
            next(); /* consume '.' */
            if (tok >= TOK_UIDENT) {
                const char *sym_name = get_tok_str(tok, NULL);
                char prefixed_name[512];
                int prefixed_tok;
                snprintf(prefixed_name, sizeof(prefixed_name), "%s_%s", module_name, sym_name);
                prefixed_tok = tok_alloc(prefixed_name, strlen(prefixed_name))->tok;
                s = sym_find(prefixed_tok);
                if (s) {
                    t = prefixed_tok;
                    next(); /* consume symbol name */
                }
            }
        }
        if (!s || IS_ASM_SYM(s)) {
            ...
            if (tok != '(')
                nld_error("'%s' undeclared", name);
            /* for simple function calls, we tolerate undeclared
               external reference to int() function */
            if (nld_state->warn_implicit_function_declaration
#ifdef NILDO_TARGET_PE
                ...
                nld_warning("implicit declaration of function '%s'", name);
    ...
    while (1) {
	    ...
	    s = find_field(&vtop->type, tok);
            if (!s)
                nld_error("field not found: %s",  get_tok_str(tok & ~SYM_FIELD, &tokc));
...
#ifdef CONFIG_NILDO_BCHECK
                /* if bound checking, the referenced pointer must be checked */
                if (nld_state->do_bounds_check && (vtop->r & VT_VALMASK) != VT_LOCAL)
                    vtop->r |= VT_MUSTBOUND;
#endif
            }
            next();
            ...
            if (sa)
                nld_error("too few arguments to function");
            skip(')');
            gfunc_call(nb_args);
...
ST_FUNC int expr_const(void)
{
    ...
    if (c != wc && (unsigned)c != wc)
        nld_error("constant exceeds 32 bit");
    return c;
}
...
static void block(int *bsym, int *csym, int is_expr)
{
    ...
    /* generate line number info */
    if (nld_state->do_debug)
        nld_debug_line(nld_state);
	...
	nocode_wanted = saved_nocode_wanted;
        ...
        for (b = 1; b < sw.n; b++)
            if (sw.p[b - 1]->v2 >= sw.p[b]->v1)
                nld_error("duplicate case value");
    ...
    if (tok == TOK_CASE) {
        struct case_t *cr = nld_malloc(sizeof(struct case_t));
        if (!cur_switch)
            expect("switch");
	nocode_wanted &= ~0x20000000;
        next();
    ...
    if (tok == TOK_GOTO) {
	    ...
	    next();
            ...
            if (s) {
                if (s->r == LABEL_DEFINED)
                    nld_error("duplicate label '%s'", get_tok_str(s->v, NULL));
                gsym(s->jnext);
                s->r = LABEL_DEFINED;
            } else {
	    ...
	    nocode_wanted &= ~0x20000000;
            if (tok == '}') {
                nld_warning("deprecated use of label at end of compound statement");
            } else {
                if (is_expr)
                    vpop();
                block(bsym, csym, is_expr);
...
static void skip_or_save_block(TokenString **str)
{
	...
	if (tok == TOK_EOF) {
	     if (str || level > 0)
	       nld_error("unexpected end of file");
	     else
	       break;
	}
	if (str)
	   ...
	   (compound literals).  */
        if (((vtop->r & (VT_VALMASK | VT_LVAL)) != VT_CONST
		 ...
		 || vtop->sym->v < SYM_FIRST_ANOM))
            ...
            nld_error("initializer element is not constant");
        break;
    case EXPR_ANY:
        expr_eq();
        break;
...
static void init_putv(CType *type, Section *sec, unsigned long c)
{
	...
	int size, align;
        ...
        if ((vtop->r & VT_SYM)
            ...
            nld_error("initializer element is not computable at load time");
	...
	if ((vtop->r & (VT_SYM|VT_CONST)) == (VT_SYM|VT_CONST) &&
	    ...
	    ssec = nld_state->sections[esym->st_shndx];
		...
		*(double *)ptr = vtop->c.d;
		break;
	    case VT_LDOUBLE:
#if defined NILDO_IS_NATIVE_387
		    ...
		    *(double *)ptr = (double)vtop->c.ld;
                    ...
                    nld_error("can't cross compile long double constants");
		break;
#if PTR_SIZE != 8
	    case VT_LLONG:
                             ...
                             int first, int size_only)
{
	...
	is_compatible_unqualified_types(type, &vtop->type)) {
        ...
        if ((first && tok != TOK_LSTR && tok != TOK_STR) || 
            tok == '{') {
            if (tok != '{')
                nld_error("character array initializer must be a literal,"
	    ...
	    len = 0;
            while (tok == TOK_STR || tok == TOK_LSTR) {
                ...
                if (!size_only) {
                    if (cstr_len > nb)
                        nld_warning("initializer-string for array is too long");
                                   ...
                                   int has_init, int v, int scope)
{
    ...
    int saved_nocode_wanted = nocode_wanted;
#ifdef CONFIG_NILDO_BCHECK
    int bcheck = nld_state->do_bounds_check && !NODATA_WANTED;
#endif
...
static void gen_function(Sym *sym)
{
    ...
    vla_sp_root_loc = -1;
    /* put debug symbol */
    nld_debug_funcstart(nld_state, sym);
    ...
    nld_debug_funcend(nld_state, ind - func_ind);
...
static int decl0(int l, int is_for_loop_init, Sym *func_sym)
{
    ...
    while (1) {
        if (!parse_btype(&btype, &ad)) {
            ...
            if (tok >= TOK_UIDENT) {
               /* special test for old K&R protos without explicit int
                ...
                /* NILDO: if we're inside a module block and hit '}', just break */
                if (in_module_block && tok == '}')
                    break;
                if (tok != TOK_EOF)
                    expect("declaration");
                break;
		...
		int v = btype.ref->v;
		if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) >= SYM_FIRST_ANOM)
        	    nld_warning("unnamed struct/union that defines no instances");
                next();
                continue;
	    }
            if (IS_ENUM(btype.t)) {
		...
		type.ref = sym_push(SYM_FIELD, &type.ref->type, 0, type.ref->c);
            ...
            if ((type.t & VT_BTYPE) == VT_FUNC) {
                if ((type.t & VT_STATIC) && (l == VT_LOCAL)) {
                    nld_error("function without file scope cannot be static");
                }
            ...
            if (ad.a.dllimport || ad.a.dllexport) {
                if (type.t & (VT_STATIC|VT_TYPEDEF))
                    nld_error("cannot have dll linkage with static or typedef");
                if (ad.a.dllimport) {
                    if ((type.t & VT_BTYPE) == VT_FUNC)
                        ad.a.dllimport = 0;
                    else
            ...
            if (tok == '{') {
                if (l != VT_CONST)
                    nld_error("cannot use local functions");
                if ((type.t & VT_BTYPE) != VT_FUNC)
		   ...
		   make old style params without decl have int type */
		    ...
		    if (sym->type.t == VT_VOID)
                ...
                if ((type.t & (VT_INLINE | VT_STATIC)) == 
                    ...
                    fn = nld_malloc(sizeof *fn + strlen(filename));
                    strcpy(fn->filename, filename);
                    fn->sym = sym;
		    skip_or_save_block(&fn->func_str);
                    dynarray_add(&nld_state->inline_fns,
				 &nld_state->nb_inline_fns, fn);
                } else {
		...
		if (l == VT_CMP) {
			...
			if ((sym->v & ~SYM_FIELD) == v)
			    goto found;
		    nld_error("declaration for parameter '%s' but no such parameter",
			      get_tok_str(v, NULL));
found:
		    if (type.t & VT_STORAGE) /* 'register' is okay */
		        nld_error("storage class specified for '%s'",
				  get_tok_str(v, NULL));
		    if (sym->type.t != VT_VOID)
		        nld_error("redefinition of parameter '%s'",
				  get_tok_str(v, NULL));
		    ...
		    sym->type = type;
                    ...
                    if (sym && sym->sym_scope == local_scope) {
                        if (!is_compatible_types(&sym->type, &type)
                            ...
                            nld_error("incompatible redefinition of '%s'",
                                get_tok_str(v, NULL));
                        sym->type = type;
                    } else {
                    ...
                    if (has_init && (type.t & VT_VLA))
                        nld_error("variable length array cannot be initialized");
			...
			((type.t & VT_BTYPE) == VT_FUNC) ||
                        ...
                        if (ad.alias_target) {
                            ...
                            if (!esym)
                                nld_error("unsupported forward __alias__ attribute");
                            /* Local statics have a scope until now (for
                               warnings), remove it here.  */
                            sym->sym_scope = 0;

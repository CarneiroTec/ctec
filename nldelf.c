...
ST_FUNC void nldelf_delete(NILDOState *s1)
{
    int i;
...
# endif
    ...
    nld_free(s1->sym_attrs);

    symtab_section = NULL; /* for nldrun.c:rt_printline() */
}
...
ST_FUNC void nldelf_end_file(NILDOState *s1)
{
    ...
    tr = nld_mallocz(nb_syms * sizeof *tr);
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        if (sr->sh_type == SHT_RELX && sr->link == s) {
            ...
            for (; rel < rel_end; ++rel) {
                ...
                //if (n < 0) nld_error("internal: invalid symbol index in relocation");
...
ST_FUNC Section *new_section(NILDOState *s1, const char *name, int sh_type, int sh_flags)
{
    ...
    sec = nld_mallocz(sizeof(Section) + strlen(name));
...
ST_FUNC Section *new_symtab(NILDOState *s1,
...
ST_FUNC void section_realloc(Section *sec, unsigned long new_size)
{
    ...
    while (size < new_size)
        size = size * 2;
    data = nld_realloc(sec->data, size);
...
#endif
                nld_error_noabort("'%s' defined twice", name);
            }
        } else {
        do_patch:
                            ...
                            int type, int symbol, addr_t addend)
{
    ...
    if (!sr) {
        ...
        /* if the symtab is allocated, then we consider the relocation
           are also */
        sr = new_section(nld_state, buf, SHT_RELX, symtab->sh_flags);
...
#else
    if (addend)
        nld_error("non-zero addend on REL architecture");
#endif
}
...
ST_FUNC struct sym_attr *get_sym_attr(NILDOState *s1, int index, int alloc)
{
    ...
    if (index >= s1->nb_sym_attrs) {
        ...
        while (index >= n)
            n *= 2;
        tab = nld_realloc(s1->sym_attrs, n * sizeof(*s1->sym_attrs));
        s1->sym_attrs = tab;
...
/* In an ELF file symbol table, the local symbols must appear below
   the global and weak ones. Since NILDO cannot sort it while generating
   the code, we must do it after. All the relocation tables are also
   modified to take into account the symbol table sorting */
static void sort_syms(NILDOState *s1, Section *s)
{
    int *old_to_new_syms;
    ElfW(Sym) *new_syms;
    int nb_syms, i;
    ...
    nb_syms = s->data_offset / sizeof(ElfW(Sym));
    new_syms = nld_malloc(nb_syms * sizeof(ElfW(Sym)));
    old_to_new_syms = nld_malloc(nb_syms * sizeof(int));
...
ST_FUNC void relocate_syms(NILDOState *s1, Section *symtab, int do_resolve)
{
    ...
    for_each_elem(symtab, 1, sym, ElfW(Sym)) {
        ...
        if (sh_num == SHN_UNDEF) {
            ...
            /* Use ld.so to resolve symbol for us (for nld -run) */
            if (do_resolve) {
#if defined NILDO_IS_NATIVE && !defined NILDO_TARGET_PE
            ...
            if (sym_bind == STB_WEAK)
                ...
                nld_error_noabort("undefined symbol '%s'", name);
...
static void relocate_rel(NILDOState *s1, Section *sr)
{
    Section *s;
    ElfW_Rel *rel;
...
/* count the number of dynamic relocations so that we can reserve
   their space */
static int prepare_dynamic_rel(NILDOState *s1, Section *sr)
{
    ElfW_Rel *rel;
...
static void build_got(NILDOState *s1)
{
...
static struct sym_attr * put_got_entry(NILDOState *s1, int dyn_reloc_type,
                                       unsigned long size,
...
ST_FUNC void build_got_entries(NILDOState *s1)
{
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel) {
	       ...
	       probably created by nld_add_symbol, and thus on 64-bit
	       targets might be too far from application code.  */
            if (gotplt_entry == AUTO_GOTPLT_ENTRY) {
                if (sym->st_shndx == SHN_UNDEF) {
                    ElfW(Sym) *esym;
		    int dynindex;
                    if (s1->output_type == NILDO_OUTPUT_DLL && ! PCRELATIVE_DLLPLT)
                        continue;
		    /* Relocations for UNDEF symbols would normally need
		       ...
		       But NILDO doesn't do that (at least for exes), so we
...
static void add_init_array_defines(NILDOState *s1, const char *section_name)
{
    Section *s;
    long end_offset;
    char sym_start[1024];
...
static int nld_add_support(NILDOState *s1, const char *filename)
{
    char buf[1024];
    snprintf(buf, sizeof(buf), "%s/%s", s1->nld_lib_path, filename);
    return nld_add_file(s1, buf);
}
...
/* add nld runtime libraries */
ST_FUNC void nld_add_runtime(NILDOState *s1)
{
    nld_add_bcheck(s1);
    nld_add_pragma_libs(s1);
    /* add libc */
    if (!s1->nostdlib) {
        nld_add_library_err(s1, "nld");
#ifdef NILDO_LIBGCC
        if (!s1->static_link) {
            if (NILDO_LIBGCC[0] == '/')
                nld_add_file(s1, NILDO_LIBGCC);
            else
                nld_add_dll(s1, NILDO_LIBGCC, 0);
        }
#endif
        nld_add_support(s1, NILDO_LIBNILDO1);
        /* add crt end if not memory output */
        if (s1->output_type != NILDO_OUTPUT_MEMORY)
            nld_add_crt(s1, "crtn.o");
    }
}
...
/* add various standard linker symbols (must be done after the
   ...
   symbols)) */
static void nld_add_linker_symbols(NILDOState *s1)
{
    char buf[1024];
    int i;
    Section *s;
...
ST_FUNC void resolve_common_syms(NILDOState *s1)
{
    ...
    nld_add_linker_symbols(s1);
}

static void nld_output_binary(NILDOState *s1, FILE *f,
                              const int *sec_order)
{
    Section *s;
    int i, offset, size;
...
ST_FUNC void fill_got_entry(NILDOState *s1, ElfW_Rel *rel)
{
...
ST_FUNC void fill_got(NILDOState *s1)
{
    Section *s;
    ElfW_Rel *rel;
    int i;
...
static void fill_local_got_entries(NILDOState *s1)
{
    ElfW_Rel *rel;
...
static void bind_exe_dynsyms(NILDOState *s1)
{
    const char *name;
    int sym_index, index;
    ElfW(Sym) *sym, *esym;
...
static void bind_libs_dynsyms(NILDOState *s1)
{
    const char *name;
    int sym_index;
    ElfW(Sym) *sym, *esym;
    ...
    for_each_elem(s1->dynsymtab_section, 1, esym, ElfW(Sym)) {
        ...
        if (sym_index && sym->st_shndx != SHN_UNDEF
            ...
            if (ELFW(ST_BIND)(esym->st_info) != STB_WEAK)
                nld_warning("undefined dynamic symbol '%s'", name);
        }
    }
}
...
static void export_global_syms(NILDOState *s1)
{
    int dynindex, index;
    const char *name;
    ElfW(Sym) *sym;
...
static int alloc_sec_names(NILDOState *s1, int file_type, Section *strsec)
{
    int i;
    Section *s;
    int textrel = 0;
...
static int layout_sections(NILDOState *s1, ElfW(Phdr) *phdr, int phnum,
                           ...
                           struct dyn_inf *dyninf, int *sec_order)
{
    ...
    sh_order_index = 1;
    file_offset = 0;
    if (s1->output_format == NILDO_OUTPUT_FORMAT_ELF)
...
/* Fill the dynamic section with tags describing the address and size of
   sections */
static void fill_dynamic(NILDOState *s1, struct dyn_inf *dyninf)
{
...
/* Relocate remaining sections and symbols (that is those not related to
   dynamic linking) */
static int final_sections_reloc(NILDOState *s1)
{
    int i;
    Section *s;
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        s = s1->sections[i];
#if defined(NILDO_TARGET_I386) || defined(NILDO_MUSL)
...
static void nld_output_elf(NILDOState *s1, FILE *f, int phnum, ElfW(Phdr) *phdr,
                           int file_offset, int *sec_order)
{
...
#if !defined(NILDO_TARGET_PE) && (defined(__FreeBSD__) || defined(__FreeBSD_kernel__))
    ...
    if (file_type == NILDO_OUTPUT_EXE || file_type == NILDO_OUTPUT_DLL)
    ...
    switch(file_type) {
    default:
    case NILDO_OUTPUT_EXE:
        ehdr.e_type = ET_EXEC;
        ehdr.e_entry = get_elf_sym_addr(s1, "_start", 1);
        break;
    case NILDO_OUTPUT_DLL:
        ehdr.e_type = ET_DYN;
        ehdr.e_entry = text_section->sh_addr; /* XXX: is it correct ? */
        break;
    case NILDO_OUTPUT_OBJ:
        ehdr.e_type = ET_REL;
        break;
    }
    ehdr.e_machine = EM_NILDO_TARGET;
...
static int nld_write_elf_file(NILDOState *s1, const char *filename, int phnum,
                              ElfW(Phdr) *phdr, int file_offset, int *sec_order)
{
    ...
    if (fd < 0) {
        nld_error_noabort("could not write '%s'", filename);
        return -1;
    }
    f = fdopen(fd, "wb");
    if (s1->verbose)
    ...
    if (s1->output_format == NILDO_OUTPUT_FORMAT_COFF)
        nld_output_coff(s1, f);
    else
#endif
    if (s1->output_format == NILDO_OUTPUT_FORMAT_ELF)
        nld_output_elf(s1, f, phnum, phdr, file_offset, sec_order);
    else
        nld_output_binary(s1, f, sec_order);
    fclose(f);
...
static void tidy_section_headers(NILDOState *s1, int *sec_order)
{
    int i, nnew, l, *backmap;
    Section **snew, *s;
    ElfW(Sym) *sym;

    snew = nld_malloc(s1->nb_sections * sizeof(snew[0]));
    backmap = nld_malloc(s1->nb_sections * sizeof(backmap[0]));
...
static int elf_output_file(NILDOState *s1, const char *filename)
{
    ...
    if (file_type == NILDO_OUTPUT_OBJ)
        phnum = 0;
    else if (file_type == NILDO_OUTPUT_DLL)
        phnum = 3;
    ...
    phdr = nld_mallocz(phnum * sizeof(ElfW(Phdr)));

    /* compute number of sections */
    shnum = s1->nb_sections;

    /* this array is used to reorder sections in the output file */
    sec_order = nld_malloc(sizeof(int) * shnum);
    sec_order[0] = 0;
    ...
    if (file_type != NILDO_OUTPUT_OBJ) {
        ...
        if (dynamic) {
            ...
            if (file_type == NILDO_OUTPUT_EXE
                || (RELOCATE_DLLPLT && file_type == NILDO_OUTPUT_DLL))
                relocate_plt(s1);
	...
	tidy_section_headers(s1, sec_order);
        ...
        /* Perform relocation to GOT or PLT entries */
        if (file_type == NILDO_OUTPUT_EXE && s1->static_link)
            fill_got(s1);
        else if (s1->got)
    ...
    ret = nld_write_elf_file(s1, filename, phnum, phdr, file_offset, sec_order);
...
LIBNILDOAPI int nld_output_file(NILDOState *s, const char *filename)
{
    ...
    if (s->output_type != NILDO_OUTPUT_OBJ) {
...
ST_FUNC int nld_load_object_file(NILDOState *s1,
                                int fd, unsigned long file_offset)
{
    ...
    lseek(fd, file_offset, SEEK_SET);
    if (nld_object_type(fd, &ehdr) != AFF_BINTYPE_REL)
        goto fail1;
    /* test CPU specific stuff */
    if (ehdr.e_ident[5] != ELFDATA2LSB ||
        ehdr.e_machine != EM_NILDO_TARGET) {
    fail1:
        nld_error_noabort("invalid object file");
        return -1;
    }
    /* read sections */
    shdr = load_data(fd, file_offset + ehdr.e_shoff,
                     sizeof(ElfW(Shdr)) * ehdr.e_shnum);
    sm_table = nld_mallocz(sizeof(SectionMergeInfo) * ehdr.e_shnum);
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        if (sh->sh_type == SHT_SYMTAB) {
            if (symtab) {
                nld_error_noabort("object must contain only one symtab");
            fail:
                ret = -1;
                goto the_end;
            }
    ...
    found:
        if (sh->sh_type != s->sh_type) {
            nld_error_noabort("invalid section type");
            goto fail;
        }
    ...
    /* resolve symbols */
    old_to_new_syms = nld_mallocz(nb_syms * sizeof(int));
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        case SHT_RELX:
            ...
            for_each_elem(s, (offset / sizeof(*rel)), rel, ElfW_Rel) {
                ...
                invalid_reloc:
                    nld_error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x",
...
static int nld_load_alacarte(NILDOState *s1, int fd, int size, int entrysize)
{
        ...
        for(p = ar_names, i = 0; i < nsyms; i++, p += strlen(p)+1) {
            ...
            if(sym_index) {
			  ...
			  + sizeof(ArchiveHeader);
                    ++bound;
                    if(nld_load_object_file(s1, fd, off) < 0) {
                    fail:
                        ret = -1;
                        goto the_end;
                    }
...
ST_FUNC int nld_load_archive(NILDOState *s1, int fd)
{
    ...
    for(;;) {
        ...
        if (len == 0)
            break;
        if (len != sizeof(hdr)) {
            nld_error_noabort("invalid archive");
            return -1;
        }
        ...
        if (!strcmp(ar_name, "/")) {
            ...
            if(s1->alacarte_link)
                return nld_load_alacarte(s1, fd, size, 4);
	} else if (!strcmp(ar_name, "/SYM64/")) {
            if(s1->alacarte_link)
                return nld_load_alacarte(s1, fd, size, 8);
        } else {
            ElfW(Ehdr) ehdr;
            if (nld_object_type(fd, &ehdr) == AFF_BINTYPE_REL) {
                if (nld_load_object_file(s1, fd, file_offset) < 0)
                    return -1;
            }
        }
...
ST_FUNC int nld_load_dll(NILDOState *s1, int fd, const char *filename, int level)
{
    ...
    if (ehdr.e_ident[5] != ELFDATA2LSB ||
        ehdr.e_machine != EM_NILDO_TARGET) {
        nld_error_noabort("bad architecture");
        return -1;
    }
    ...
    dllref = nld_mallocz(sizeof(DLLReference) + strlen(soname));
    dllref->level = level;
    ...
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
        ...
        case DT_NEEDED:
            ...
            if (nld_add_dll(s1, name, AFF_REFERENCED_DLL) < 0) {
                nld_error_noabort("referenced dll '%s' not found", name);
                ret = -1;
                goto the_end;
            }
        already_loaded:
...
static int ld_next(NILDOState *s1, char *name, int name_size)
{
    int c;
    char *q;
...
static int ld_add_file(NILDOState *s1, const char filename[])
{
    if (filename[0] == '/') {
        if (CONFIG_SYSROOT[0] == '\0'
            && nld_add_file_internal(s1, filename, AFF_TYPE_BIN) == 0)
            return 0;
        filename = nld_basename(filename);
    }
    return nld_add_dll(s1, filename, 0);
}
...
static int ld_add_file_list(NILDOState *s1, const char *cmd, int as_needed)
{
    ...
    for(;;) {
        libname[0] = '\0';
        if (t == LD_TOK_EOF) {
            nld_error_noabort("unexpected end of file");
            ret = -1;
            ...
            if ((t != LD_TOK_NAME) || (filename[0] != 'l')) {
                nld_error_noabort("library name expected");
                ret = -1;
                goto lib_parse_error;
            }
            ...
            nld_error_noabort("filename expected");
            ret = -1;
            goto lib_parse_error;
        }
        if (!strcmp(filename, "AS_NEEDED")) {
            ...
            if (!as_needed) {
                ...
                if (group) {
                    ...
                    dynarray_add(&libs, &nblibs, nld_strdup(filename));
                    if (libname[0] != '\0')
                        dynarray_add(&libs, &nblibs, nld_strdup(libname));
                }
            }
        }
        t = ld_next(s1, filename, sizeof(filename));

/*
 *  i386 specific functions for NILDO assembler
 *
...
static inline int get_reg_shift(NILDOState *s1)
{
    int shift, v;
    v = asm_int_expr(s1);
    switch(v) {
...
static void parse_operand(NILDOState *s1, Operand *op)
{
    ExprValue e;
    int reg, indir;
    const char *p;
    ...
    if (tok == '%') {
        ...
        reg_error:
            nld_error("unknown register %%%s", get_tok_str(tok, &tokc));
        }
        next();
    no_skip: ;
    } else if (tok == '$') {
...
static void gen_disp32(ExprValue *pe)
{
    ...
    if (esym && esym->st_shndx == cur_text_section->sh_num) {
        /* same section: we can output an absolute value. Note
           that the NILDO compiler behaves differently here because
		    ...
		    int regi, int rmi)
{
  ...
  if (rex) {
      if (saw_high_8bit)
	  nld_error("can't encode register %%%ch when REX prefix is required",
...
ST_FUNC void asm_opcode(NILDOState *s1, int opcode)
{
    ...
    for(;;) {
        ...
        if (nb_ops >= MAX_OPERANDS) {
            nld_error("incorrect number of operands");
        }
        parse_operand(s1, pop);
        if (tok == ':') {
           if (pop->type != OP_SEG || seg_prefix)
               nld_error("incorrect prefix");
           seg_prefix = segment_prefixes[pop->reg];
           next();
           parse_operand(s1, pop);
           if (!(pop->type & OP_EA)) {
               nld_error("segment prefix must be followed by memory reference");
           }
        }
        pop++;
        nb_ops++;
...
again:
    ...
    if (pa->sym == 0) {
        if (opcode >= TOK_ASM_first && opcode <= TOK_ASM_last) {
            ...
            nld_error("bad operand with opcode '%s'",
		...
		goto again;
            ...
            nld_error("unknown opcode '%s'", ts->str);
        }
    }
	   ...
	   are such opcodes.  */
        ...
        if (s == autosize) {
	        ...
	        s = NBWLX - 2;
                ...
                nld_error("cannot infer opcode suffix");
        }
    }
	...
	esym = elfsym(ops[0].e.sym);
	    ...
	    if (v == 0xeb) /* jmp */
	        ...
	        nld_error("invalid displacement");
        }
    }
...
static inline int constraint_priority(const char *str)
{
	...
	case 'e':
        ...
        case 'g':
            ...
            break;
        default:
            nld_error("unknown constraint '%c'", c);
            pr = 0;
        }
        if (pr > priority)
            priority = pr;
                                    ...
                                    int *pout_reg)
{
    ...
    for(i=0;i<nb_operands;i++) {
        ...
        if (isnum(*str) || *str == '[') {
            ...
            if ((unsigned)k >= i || i < nb_outputs)
                nld_error("invalid reference in constraint %d ('%s')",
                      i, str);
            op->ref_index = k;
            if (operands[k].input_index >= 0)
                nld_error("cannot reference twice the same operand");
	...
	if (op->reg >= 0) {
	    if (is_reg_allocated(op->reg))
	        nld_error("asm regvar requests register that's taken already");
	    reg = op->reg;
	    goto reg_found;
	}
    try_next:
        ...
        case '&':
            if (j >= nb_outputs)
                nld_error("'%c' modifier can only be applied to outputs", c);
            reg_mask = REG_IN_MASK | REG_OUT_MASK;
            goto try_next;
        case 'A':
	...
	case 'e':
        ...
        case 'g':
            ...
            break;
        default:
            nld_error("asm constraint %d ('%s') could not be satisfied",
                  j, op->constraint);
            break;
        }
    ...
    *pout_reg = -1;
    for(i=0;i<nb_operands;i++) {
        ...
        if (op->reg >= 0 &&
            ...
            nld_error("could not find free output register for reloading");
        reg_found2:
            *pout_reg = reg;
            break;
        }
                              ...
                              SValue *sv, int modifier)
{
    ...
    no_offset:;
        ...
        reg = r & VT_VALMASK;
        if (reg >= VT_CONST)
            nld_error("internal compiler error");
        ...
        if (modifier == 'b') {
            if (reg >= 4)
                nld_error("cannot use byte register");
            size = 1;
        } else if (modifier == 'h') {
            if (reg >= 4)
                nld_error("cannot use byte register");
...
ST_FUNC void asm_clobber(uint8_t *clobber_regs, const char *str)
{
	...
	;
        ...
        nld_error("invalid clobber register '%s'", str);
    }
    clobber_regs[reg] = 1;
}

...
static void set_options(NILDOState *s, int argc, char **argv)
{
    int i;
    for (i = 1; i < argc; ++i) {
        char *a = argv[i];
...
static int run_callback(const char *src, callback_type callback) {
  ...
  if (nld_set_output_type(s, NILDO_OUTPUT_MEMORY) == -1)
    return -1;
  if (nld_compile_string(s, src) == -1)
    return -1;
  if (nld_relocate(s, NILDO_RELOCATE_AUTO) == -1)
    return -1;
  
  ptr = nld_get_symbol(s, "f");
  if (!ptr)
    return -1;
  result = callback(ptr);
 ...
 * address, thus causing a success message even though NILDO actually

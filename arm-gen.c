/*
 *  ARMv4 code generator for NILDO
 *
...
#if defined(NILDO_ARM_EABI) && !defined(NILDO_ARM_VFP)
#error "Currently Nildo only supports float computation with VFP instructions"
#endif
...
#ifndef NILDO_CPU_VERSION
# define NILDO_CPU_VERSION 5
#endif
...
#ifndef NILDO_ARM_HARDFLOAT
    nld_warning("soft float ABI currently not supported: default to softfp");
#endif
}
#else
...
{
#if 0
#if !defined (NILDO_ARM_VFP)
    nld_warning("Support for FPA is deprecated and will be removed in next"
                " release");
#endif
#if !defined (NILDO_ARM_EABI)
    nld_warning("Support for OABI is deprecated and will be removed in next"
                " release");
#endif
#endif
}
...
const char *default_elfinterp(struct NILDOState *s)
{
...
void o(uint32_t i)
{
  ...
  ind1 = ind + 4;
  if (!cur_text_section)
    nld_error("compiler error! This happens f.ex. if the compiler\n"
...
uint32_t encbranch(int pos, int addr, int fail)
{
  ...
  if(addr>=0x1000000 || addr<-0x1000000) {
    if(fail)
      nld_error("FIXME: function bigger than 32MB");
    return 0;
  }
...
static uint32_t vfpr(int r)
{
  if(r<TREG_F0 || r>TREG_F7)
    nld_error("compiler error! register %i is no vfp register",r);
  return r - TREG_F0;
}
#else
static uint32_t fpr(int r)
{
  if(r<TREG_F0 || r>TREG_F3)
    nld_error("compiler error! register %i is no fpa register",r);
  return r - TREG_F0;
}
#endif
...
static uint32_t intr(int r)
{
  ...
  if (r >= TREG_SP && r <= TREG_LR)
    return r + (13 - TREG_SP);
  nld_error("compiler error! register %i is no int register",r);
}
...
static uint32_t mapcc(int cc)
{
  ...
  nld_error("unexpected condition code");
...
/* Assign parameters to registers and stack with alignment according to the
   ...
   Note: this function allocated an array in plan->pplans with nld_malloc. It
...
static int assign_regs(int nb_args, int float_abi, struct plan *plan, int *todo)
{
  ...
  ncrn = nsaa = 0;
  *todo = 0;
  plan->pplans = nld_malloc(nb_args * sizeof(*plan->pplans));
...
again:
  ...
  /* second pass to restore registers that were saved on stack by accident.
     Maybe redundant after the "lvalue_save" patch in nldgen.c:gv() */
  if (++pass < 2)
    goto again;
...
void gen_opi(int op)
{
  ...
  switch(c) {
    ...
    case 3:
      ...
      break;
    default:
      nld_error("gen_opi %i unimplemented!",op);
  }
}
...
/* bug (intention?) in Linux FPU emulator
   doesn't set carry if equal */
	switch(op) {
	  ...
	  case TOK_ULE:
	  case TOK_UGT:
            nld_error("unsigned comparison on floats?");
	    break;
	  case TOK_LT:
            op=TOK_Nset;
	    break;
...
ST_FUNC void gen_cvt_itof1(int t)
{
  ...
  nld_error("unimplemented gen_cvt_itof %x!",vtop->type.t);
}
...
void gen_cvt_ftoi(int t)
{
  ...
  nld_error("unimplemented gen_cvt_ftoi!");
}
...
#endif
    if (align & (align - 1))
        nld_error("alignment is not a power of 2: %i", align);

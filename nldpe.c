/*
 *  NILDOPE.C - PE file output for the NILDO Compiler
 *
...
#elif defined NILDO_TARGET_I386
# define ADDR3264 DWORD
# define PE_IMAGE_REL IMAGE_REL_BASED_HIGHLOW
...
static const char *pe_export_name(NILDOState *s1, ElfW(Sym) *sym)
{
...
static int pe_find_import(NILDOState * s1, ElfW(Sym) *sym)
{
    char buffer[200];
    const char *s, *p;
    int sym_index = 0, n = 0;
...
#define CHARACTERISTICS_DLL 0x230F
    ...
    if (NULL == op) {
        nld_error_noabort("could not write '%s': %s", pe->filename, strerror(errno));
        return -1;
    }
        ...
        for (k = 0, n = p->sym_count; k <= n; ++k) {
            if (k < n) {
                ...
                if (dllref)
                    ...
                    ordinal = 0, v = imp_sym->st_value; /* address from nld_add_symbol() */
                ...
                if (pe->type == PE_RUN) {
                    ...
                    if (!v)
                        nld_error_noabort("can't build symbol '%s'", name);
                } else
#endif
                if (ordinal) {
...
static void pe_build_exports(struct pe_info *pe)
{
    ...
    pe_align_section(pe->thunk, 16);
    dllname = nld_basename(pe->filename);
    ...
    strcpy(nld_fileextension(buf), ".def");
    op = fopen(buf, "wb");
    if (NULL == op) {
        nld_error_noabort("could not create '%s': %s", buf, strerror(errno));
    } else {
...
static int pe_assign_addresses (struct pe_info *pe)
{
    ...
    section_order = nld_malloc(pe->s1->nb_sections * sizeof (int));
    ...
    pe->sec_info = nld_mallocz(o * sizeof (struct section_info));
    addr = pe->imagebase + 1;
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (type == STT_FUNC) {
                ...
                /* nld_realloc might have altered sym's address */
                sym = (ElfW(Sym) *)symtab_section->data + sym_index;
        ...
        not_found:
            if (ELFW(ST_BIND)(sym->st_info) == STB_WEAK)
                ...
                continue;
            nld_error_noabort("undefined symbol '%s'%s", name,
...
static void pe_print_sections(NILDOState *s1, const char *fname)
{
    Section *s;
    FILE *f;
    int i;
...
static int add_dllref(NILDOState *s1, const char *dllname)
{
    DLLReference *dllref;
    int i;
    for (i = 0; i < s1->nb_loaded_dlls; ++i)
        if (0 == strcmp(s1->loaded_dlls[i]->name, dllname))
            return i + 1;
    dllref = nld_mallocz(sizeof(DLLReference) + strlen(dllname));
...
found:
    ...
    for (i = 0; i < ied.NumberOfNames; ++i) {
        ...
        for (l = 0;;) {
            if (n+1 >= n0)
                p = nld_realloc(p, n0 = n0 ? n0 * 2 : 256);
...
static int pe_load_res(NILDOState *s1, int fd)
{
...
/* ------------------------------------------------------------- */
static int pe_load_def(NILDOState *s1, int fd)
{
...
/* ------------------------------------------------------------- */
static int pe_load_dll(NILDOState *s1, const char *filename)
{
    char *p, *q;
    int index, ret;

    ret = nld_get_dllexports(filename, &p);
    if (ret) {
        return -1;
    } else if (p) {
        index = add_dllref(s1, nld_basename(filename));
...
ST_FUNC int pe_load_file(struct NILDOState *s1, const char *filename, int fd)
{
    int ret = -1;
    char buf[10];
    if (0 == strcmp(nld_fileextension(filename), ".def"))
...
/* ------------------------------------------------------------- */
#ifdef NILDO_TARGET_X86_64
static unsigned pe_add_uwwind_info(NILDOState *s1)
{
    if (NULL == s1->uw_pdata) {
        s1->uw_pdata = find_section(nld_state, ".pdata");
...
static void pe_add_runtime(NILDOState *s1, struct pe_info *pe)
{
    const char *start_symbol;
    int pe_type = 0;
    int unicode_entry = 0;
    ...
    if (NILDO_OUTPUT_DLL == s1->output_type) {
        pe_type = PE_DLL;
        /* need this for 'nldelf.c:relocate_section()' */
        s1->output_type = NILDO_OUTPUT_EXE;
    }
    else {
        pe_type = PE_EXE;
    ...
    start_symbol =
        NILDO_OUTPUT_MEMORY == s1->output_type
    ...
    /* grab the startup code from libnld1 */
#ifdef NILDO_IS_NATIVE
    if (NILDO_OUTPUT_MEMORY != s1->output_type || s1->runtime_main)
#endif
    ...
    if (0 == s1->nostdlib) {
        static const char *libs[] = {
            NILDO_LIBNILDO1, "msvcrt", "kernel32", "", "user32", "gdi32", NULL
        };
        const char **pp, *p;
        for (pp = libs; 0 != (p = *pp); ++pp) {
            if (0 == *p) {
                if (PE_DLL != pe_type && PE_GUI != pe_type)
                    break;
            } else if (pp == libs && nld_add_dll(s1, p, 0) >= 0) {
                continue;
            } else {
                nld_add_library_err(s1, p);
            }
        }
    }

    if (NILDO_OUTPUT_MEMORY == s1->output_type)
        pe_type = PE_RUN;
    pe->type = pe_type;
    pe->start_symbol = start_symbol;
}

static void pe_set_options(NILDOState * s1, struct pe_info *pe)
{
    if (PE_DLL == pe->type) {
...
ST_FUNC int pe_output_file(NILDOState *s1, const char *filename)
{
    ...
    if (ret)
        ...
        pe.start_addr = (DWORD)
            ((uintptr_t)nld_get_symbol_err(s1, pe.start_symbol)
                - pe.imagebase);
        if (s1->nb_errors)
            ret = -1;
        else
            ret = pe_write(&pe);
        nld_free(pe.sec_info);
    } else {
#ifdef NILDO_IS_NATIVE

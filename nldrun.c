/*
 *  NILDO - NILDO Compiler - Support for -run switch
 *
...
static int nld_relocate_ex(NILDOState *s1, void *ptr, addr_t ptr_diff);

#ifdef _WIN64
static void *win64_add_function_table(NILDOState *s1);
...
LIBNILDOAPI int nld_relocate(NILDOState *s1, void *ptr)
{
    ...
    if (NILDO_RELOCATE_AUTO != ptr)
        return nld_relocate_ex(s1, ptr, 0);

    size = nld_relocate_ex(s1, NULL, 0);
    if (size < 0)
        return -1;
...
#ifdef HAVE_SELINUX
{
    ...
    void *prx;
    char tmpfname[] = "/tmp/.nldrunXXXXXX";
	...
	nld_error("nldrun: could not map memory");
    ...
    ptr = nld_malloc(size);
#endif
    nld_relocate_ex(s1, ptr, ptr_diff); /* no more errors expected */
...
#endif
        nld_free(s1->runtime_mem[i]);
#endif
    }
    nld_free(s1->runtime_mem);
}

/* launch the compiled program with the given arguments */
LIBNILDOAPI int nld_run(NILDOState *s1, int argc, char **argv)
{
    int (*prog_main)(int, char **);

    s1->runtime_main = "Início";
    if ((s1->dflag & 16) && !find_elf_sym(s1->symtab, s1->runtime_main))
        return 0;
    if (nld_relocate(s1, NILDO_RELOCATE_AUTO) < 0)
        return -1;
    prog_main = nld_get_symbol_err(s1, s1->runtime_main);
    ...
    if (s1->do_bounds_check) {
        ...
        /* set error function */
        rt_bound_error_msg = nld_get_symbol_err(s1, "__bound_error_msg");
        /* XXX: use .init section so that it also work in binary ? */
        bound_init = nld_get_symbol_err(s1, "__bound_init");
        bound_exit = nld_get_symbol_err(s1, "__bound_exit");
        bound_new_region = nld_get_symbol_err(s1, "__bound_new_region");
        bound_delete_region = nld_get_symbol_err(s1, "__bound_delete_region");
...
#  endif
    ...
    if (mprotect((void *)start, end - start, PROT_READ | PROT_WRITE | PROT_EXEC))
        nld_error("mprotect failed: did you mean to configure --with-selinux?");
# endif
# if defined NILDO_TARGET_ARM || defined NILDO_TARGET_ARM64
...
static void set_exception_handler(void)
{
    ...
    /* install NILDO signal handlers to print debug info on fatal
       runtime errors */
...
static int rt_get_caller_pc(addr_t *paddr, ucontext_t *uc, int level)
{
    ...
    if (level == 0) {
        ...
        *paddr = uc->uc_mcontext.arm_pc;
        ...
        return -1;
#endif
        /* XXX: specific to nld stack frames */
        if (fp < sp + 12 || fp & 3)
            return -1;
        for(i = 1; i < level; i++) {
            sp = ((addr_t *)fp)[-2];
...
static NILDOSyms nld_syms[] = {
#if !defined(CONFIG_NILDOBOOT)
#define NILDOSYM(a) { #a, &a, },
    NILDOSYM(printf)
    NILDOSYM(fprintf)
    NILDOSYM(fopen)
    NILDOSYM(fclose)
...
#endif /* CONFIG_NILDO_STATIC */
#endif /* NILDO_IS_NATIVE */
/* ------------------------------------------------------------- */
